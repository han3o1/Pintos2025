    ...
    case SYS_CREATE: // 4
      {
        ...
        size = *(unsigned *)(f->esp + 8);          // Get initial size
        return_code = create(file_name, size);     // Create the file
...
int read(int fd, void *buffer, unsigned size) {
  ...
  if(file_d && file_d->file_struct) {
    int bytes_read = file_read(file_d->file_struct, buffer, size); // Read from file
    lock_release(&filesys_lock);
    return bytes_read;
  }
