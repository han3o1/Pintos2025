...
static struct list sleep_list; /* alarm clock - 
List of sleeping threads that are blocked until a specific tick. */ 

typedef int fixed_point_t; /*MLFQ - 
Type alias for fixed-point arithmetic used in MLFQ scheduling.*/
static fixed_point_t load_avg; /*MLFQ -
System load average, updated every second as part of MLFQ.*/
...
/* Idle thread. */
struct thread *idle_thread; /*MLFQ - 
Pointer to the idle thread (now non-static for external access).*/
...
void mlfqs_update_priority (struct thread *t, void *aux UNUSED); /*MLFQ - 
/* Recalculate and update the priority of the given thread based on recent_cpu and nice.*/

void mlfqs_update_recent_cpu (struct thread *t, void *aux UNUSED); /*MLFQ -
Update the recent_cpu value of the given thread based on load_avg and nice.*/

void mlfqs_update_recent_cpu_all (void); /*MLFQ -
Update recent_cpu for all threads (called once per second).*/

void mlfqs_update_priority_all (void); /*MLFQ -
Recalculate priorities for all threads (called every 4 ticks).*/

void mlfqs_update_load_avg (void); /*MLFQ - 
Recalculate system load average (called once per second).*/
...
/* alarm clock - thread.c #235-271*/
/* Comparison function used to sort sleeping threads by wakeup_tick. 
   Returns true if thread a's wakeup_tick is earlier than thread b's. */
static bool
 ...
 /* Puts the current thread to sleep until the given wakeup_tick.
   The thread is added to the sleep_list and will be unblocked later. */
 void 
 thread_sleep(int64_t wakeup_tick) 
 {
   ...
   /*priority scheduling -
   Insert alignment into sleep_list based on wakeup_tick*/
 ...
 /* Wakes up all threads whose wakeup_tick is less than or equal to current_tick.
   These threads are removed from the sleep_list and unblocked. */
 void 
...
thread_unblock (struct thread *t) 
{
  ...
  /*MLFQ - Do not add the idle thread to the ready_list.
  Insert thread into ready_list in order of priority, 
  unless it's the idle thread.*/
  if (t != idle_thread) 
    
    /*Priority Scheduling - 
    Insert the thread into the ready list in descending priority order.*/
...
thread_yield (void) 
{
  ...
  if (cur != idle_thread) 
    ...
    /*priority scheduling - 
    Yielding thread is re-inserted into the ready list based on its priority.*/
...
thread_set_priority (int new_priority) 
{
  ...
  /*priority scheduling - 
  Update the thread's initial priority and refresh its effective priority,
  considering any current donations.*/
  struct thread *cur = thread_current(); 
  cur->init_priority = new_priority;
  refresh_priority();
 
  /*priority scheduling - 
  If there is a thread in the ready list with higher priority than the current thread,
  yield the CPU to allow preemption.*/
...
/*MLFQ - Sets the current thread's nice value and updates its priority.
   If the thread's new priority is lower than the highest-priority ready thread,
   it yields the CPU. */
void
thread_set_nice (int nice) 
{
  ...
  cur->nice = nice; // Set the thread's nice value.
...
/*MLFQ - Returns system load average multiplied by 100, 
...
/*MLFQ - Returns the current thread's recent_cpu multiplied by 100, 
rounded to nearest int.*/
int
...
init_thread (struct thread *t, const char *name, int priority)
{
  ...
  /*priority scheduling - Initialize the thread's priority and donation-related fields.*/
...
/*priority scheduling - 
Comparison function used to sort threads by priority in descending order.
...
/*Propagates priority donation through the chain of locks.
If a thread is waiting on a lock held by a lower-priority thread,
it donates its priority to the holder. This is repeated up to 8 levels deep
to prevent infinite donation chains.*/
...
// Restore priorities to initial values and reflect the highest value of donated priorities
void 
...
/*MLFQ*/
#define F (1 << 14) // Scaling factor: 1 fixed-point unit = 1 / 2^14
...
Recalculates the priority of thread t based on MLFQ formula:
   ...
   The result is clamped between PRI_MIN and PRI_MAX.
*/
void
...
/* Recalculates recent_cpu for thread t using:
...
/* Updates recent_cpu for all threads using thread_foreach. */
void
...
/* Updates priorities for all threads based on their recent_cpu and nice values. */
void
...
/* Updates the system load_avg based on the number of ready threads.
