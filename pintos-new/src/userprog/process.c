...
start_process (void *file_name_)
{
  ...
  /* Start the user process by simulating a return from an
     ...
     and jump to it. */
    /*
   * [5] Execute the user process:
   * - Set up the stack pointer (%esp) to point to if_
   * - Call intr_exit() to switch to user mode
   */   
  asm volatile ("movl %0, %%esp; jmp intr_exit" : : "g" (&if_) : "memory");
  NOT_REACHED ();  // Execution should never reach here
}
...
process_wait (tid_t child_tid)
{
  ...
  // Acquire the lock for synchronization with the child
...
process_exit (void)
{
  // Get the current thread (representing the current process)
  struct thread *cur = thread_current ();
  uint32_t *pd;
  ...
  // Switch back to kernel-only page directory and destroy the process's page directory   
  pd = cur->pagedir;
  if (pd != NULL)
    {
      ...
      cur->pagedir = NULL;  // Set pagedir to NULL before switching to kernel to avoid race conditions 
      pagedir_activate (NULL);  // Switch to kernel-only page directory
      pagedir_destroy (pd);  // Destroy current process's page directory
    }
...
load (const char *file_name, void (**eip) (void), void **esp)
{
  ...
  * - Prevents other processes or threads from modifying the file while it's running.
  * - Ensures the running program’s code remains stable and unaltered.
  */
  file_deny_write(file);
  /*
  * Save the currently executing file to the thread structure:
  * - This will be used later to allow writes again after the program ends.
  */
