...
thread_unblock (struct thread *t) 
{
  ...
  list_insert_ordered(&ready_list, &t->elem, compare_priority, NULL); /*Priority Scheduling*/
...
thread_yield (void) 
{
  ...
  old_level = intr_disable ();
  if (cur != idle_thread) 
    list_insert_ordered(&ready_list, &cur->elem, compare_priority, NULL); /*Priority Scheduling*/
...
thread_sleep(int64_t ticks) {
  ...
  list_insert_ordered(&sleep_list, &cur->elem,
                      (list_less_func *) &wakeup_tick_less, NULL); // 정렬된 삽입
  thread_block(); // 기능 1: Alarm Clock - Block 상태로 전환
  intr_set_level(old_level);
}
...
thread_wakeup(int64_t ticks) {
  ...
  while (e != list_end(&sleep_list)) {
    ...
    if (t->wakeup_tick <= ticks) {
      e = list_remove(e);
      thread_unblock(t); // 기능 1: Alarm Clock - Ready 상태로 전환
    } else {
      break;
    }
  }
...
void donate_priority(void) {
  ...
  while (cur->wait_on_lock && depth < 8) {
